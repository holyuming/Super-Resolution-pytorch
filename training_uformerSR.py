...
from models.uformerSR import UformerSR
...
if __name__ == '__main__':
    ...
    num_heads   = [1, 2, 4, 8, 16, 16, 8, 4, 2] # [1, 2, 4, 8, 16, 16, 8, 4, 2]
    model = UformerSR(img_size=input_size, embed_dim=16,depths=depths,
                    win_size=8, mlp_ratio=3., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False, num_heads=num_heads).to(device)

    macs, params = profile(model, inputs=(img, ))
    print(f'FLOPs: {macs * 2 / 1e9} G, for input size: {img.shape}')

    # model path
    model_save_path = 'pretrained_weight/uformerSR/bullshit_training_uformerSR.pth'
    model_checkpoint_path = 'checkpoints/uformerSR.pt'
